...
export default function NoteForm({ note }: { note?: NoteType }) {
  ...
  return (
    <form action={formAction} className='flex flex-col gap-4 w-[20rem]'>
      ...
      <input autoComplete='off' name="title" type="text" placeholder="Titulo" className="input input-bordered w-full max-w-xs" defaultValue={note ? note.title : formState?.prevState?.title} />
      <p className='text-orange-500 italic min-h-6'>{formState?.errors?.title}</p>
      <textarea className="textarea textarea-bordered" placeholder="Contenido" name="content" defaultValue={note ? note.content : formState?.prevState?.content} />
